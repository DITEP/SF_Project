...
class HAN(Model):
    ...
    def build_word_encoder(self, max_words, embedding_matrix, encoding_dim=200):
        ...
        """
        assert encoding_dim % 2 == 0, "Encoding dimension should be even"
    ...
    def _build_network(self):
        ...
        out_tensor = Dense(
            self.output_size, name='class_prediction', activation = 'softmax',
    ...
    def predict_word_attention(self, X):
        ...
        :return: 2d array (num_obs, max_sentences) containing
            the attention weights for each sentence
        """
        wrapper = self.get_layer('word_attention')
        prev_tensor = wrapper.input
        att_layer = wrapper.layer

        # Create a temporary dummy layer to hold the
        # attention weights tensor
        dummy_layer = TimeDistributed(Lambda(
            lambda x: att_layer._get_attention_weights(x)
        ))(prev_tensor)
